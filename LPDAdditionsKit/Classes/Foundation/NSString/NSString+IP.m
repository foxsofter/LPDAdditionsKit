...
+ (NSString *)lpd_getIPAddress:(BOOL)preferIPv4 {
    ...
    NSDictionary *addresses = [self lpd_getIPAddresses];
    ...
    [searchArray enumerateObjectsUsingBlock:^(NSString *key, NSUInteger idx, BOOL *stop) {
        ...
        //筛选出IP地址格式
        if ([self lpd_isValidatIP:address])
            *stop = YES;
    }];
    return address ? address : @"0.0.0.0";
}

+ (BOOL)lpd_isValidatIP:(NSString *)ipAddress {
...
+ (NSDictionary *)lpd_getIPAddresses {
